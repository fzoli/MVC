(function($){
    ...
    function getReturn(response, options) {
        return $(response).find(options.nodeReturn).attr(options.attrValue);
    }
    ...
    $.ModelChangeListener = function(options) {
        ...
        var /*Timer*/ timer = $.timer(function(){
            ...
            $.ajax({
                ...
                success: function(response) {
                    errorCounter = 0;
                    var closed = $(response).find(options.nodeClosed);
                    var textNewId = closed.attr(options.attrNewId);
                    if (textNewId != null) {
                        var time = closed.attr(options.attrInitTime);
                        if (initTime != null && initTime != time) options.serverRestart();
                        var isNewId = textNewId == 'true';
                        if (isNewId) {
                            listenerId = closed.attr(options.attrListenerId);
                            initTime = time;
                            regAllModel();
                        }
                    }
                    var reason = closed.attr(options.attrReason);
                    if (reason != null) {
